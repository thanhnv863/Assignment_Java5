...
<div class="row">
    ...
    <div class="col-7">
        ...
        <c:forEach items="${mauSac}" var="mauSac">
            <a href="/mua-hang/${ctsp.id}/${mauSac.id}"
               class=" ${ctsp.mauSac.ten == mauSac.ten ? 'btn btn-danger' : 'btn btn-outline-dark'}"
               role="button">${mauSac.ten}</a>
        </c:forEach> <br>
        Kích cỡ: <br>
        <c:forEach items="${ctspKichCo}" var="ctspKichCo">
            <a href="/mua-hang/${ctspKichCo.id}" value="${ctspKichCo.kichCo}"
               class="${ctsp.kichCo == ctspKichCo.kichCo ? 'btn btn-dark' : 'btn btn-outline-dark'}"
               role="button">${ctspKichCo.kichCo}</a>
        </c:forEach>
        <h2 class="font-weight-bold" style="color: crimson">${ctsp.donGia} $</h2>
        <h5>${ctsp.soLuong} sản phẩm có sẵn</h5>
        <div class="row">
            <div class="col-2">
                <form action="/add-cart/${ctsp.id}" method="get">
                    Số lượng: <br><input type="number" name="soLuong" class="form-control" style="width: 200px"
                                         placeholder="1"> <br>
            ...
            <div class="col-3" style="margin-top: 87px;">
                <a href="/trang-chu/khach-hang" class="btn btn-outline-primary" role="button" style="width: 200px">Tiếp
                    tục mua hàng</a>
            </div>
        </div>
